...
import { ShieldAlert, Activity, AlertCircle, Search, ChevronLeft, ChevronRight } from 'lucide-react';
...
const LABEL_CLASS =
    'block text-xs font-bold uppercase tracking-widest text-muted mb-2 ml-1';
...
export default function AdminPanel() {
    ...
    async function handleAccountAction(action: 'freeze' | 'unfreeze') {
        if (!accountTarget || !accountAsset || !accountSecret) {
            notifyError(
                'Missing fields',
                'Target account, asset code, and issuer secret are required.'
            );
            return;
        }
    ...
    async function handleStatusCheck() {
        if (!statusTarget || !statusAsset || !statusIssuer) {
            notifyError(
                'Missing fields',
                'Target account, asset code, and asset issuer are required.'
            );
            return;
        }
                ...
                {activeTab === 'account' && (
                    <div className="flex flex-col gap-6 max-w-2xl">
                        ...
                        <p className="text-sm text-muted">
                            Instantly block or restore an individual account's ability to transact with your asset.
                        </p>
                ...
                {activeTab === 'global' && (
                    <div className="flex flex-col gap-6 max-w-2xl">
                        ...
                        <div className="bg-red-500/10 border border-red-500/30 p-4 rounded-xl text-red-400 text-sm">
                            <strong>WARNING:</strong> This will freeze ALL accounts holding this asset.
                            Reserve for systemic security breaches only.
                        </div>
